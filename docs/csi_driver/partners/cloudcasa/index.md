...
stringData:
  ...
  servicePort: "8080"
  backend: <array-management-ip>               # array IP / endpoint
...
Each array must be linked to its cluster (a *cluster storage system*) so
...
A DR plan ties together the source and destination clusters, their storage-system
links, the inventory PVC, the inventory schedule, and the protected workloads.
...
The inventory discovers the replicated volumes, their Remote Copy groups, and the
...
No volume data is copied during failover — the data is already on the destination
array via Remote Copy.
