...
!!! note
    The `accessMode` may be set to `ReadWriteOnce`, `ReadWriteMany` or `ReadOnlyMany`. It's expected that the application handles read/write IO, potential locking and access in the event of simultaneous block access from multiple nodes.
