...
| Supported platform in an air-gapped enviornment | The [Helm chart](#helm) using the air-gapped procedure |
...
!!! note
    Only HPE CSI Driver 1.4.0 and later is supported using this methodology.

Create a working directory and export an environment variable to the private registry and what versions being installed. In this example we'll use HPE CSI Driver 1.4.0 on Kubernetes 1.20. Available variants are available in the [co-deployments GitHub repo](https://github.com/hpe-storage/co-deployments/tree/master/yaml/csi-driver).
...
!!! tip
    Depending on what kind of private registry being used, the base repositories `hpe-storage` and `k8scsi` might need to be created and given write acces to the user pushing the images.
