...
| onPremCloudCA       | A Base64-encoded CA certificate for on-premise HPE Data Services Cloud Console deployments. Required for on-premise (Deneb) setups where the DSCC instance uses a certificate signed by a private or non-public Certificate Authority that is not present in the cluster's trusted certificate store. If the CA certificate is already available in the cluster's truststore, this parameter can be omitted.
...
stringData:
  ...
  # On-premise (Deneb) deployments: Base64-encoded CA certificate (required if not in cluster truststore)
  # onPremCloudCA: <base64-encoded-ca-certificate>
```

!!! note
    The `onPremCloudCA` parameter is required for on-premise (Deneb) deployments where the DSCC CA certificate is not present in the cluster's trusted certificate store. If the cluster already has the CA certificate in its truststore, this parameter can be omitted. It is not applicable for cloud-hosted DSCC environments.
