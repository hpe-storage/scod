...
| locking                       | Text           | Enable object locking on the bucket. The only accepted value is `Enabled` (case-insensitive). Requires `versioning` to be `Enabled`. Object locking cannot be disabled once enabled on a bucket. Example: `.parameters.locking: Enabled`. |
...
| defaultRetentionInterval      | Text           | The default retention duration for object locking. Format is `<number><unit>` where unit is `d` for days, `m` for months, or `y` for years. Only applicable when `locking` is `Enabled`. Example: `.parameters.defaultRetentionInterval: 30d`. |

!!! warning
    When using `m` (months) in `defaultRetentionInterval`, each month is treated/counted as 30 days, not as a calendar month. So plan retention durations accordingly.
